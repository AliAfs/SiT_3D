...
class PatchEmbed_3D(nn.Module):
    def __init__(self, volume_size=(21, 64, 64), patch_size=(7, 16, 16), in_chans=1, embed_dim=64):
        super().__init__()
...
class VisionTransformer(nn.Module):
    def __init__(self, volume_size=(21, 64, 64), patch_size=(7,16,16), in_chans=1, num_classes=0, embed_dim=768, depth=12,
    ...
    def forward(self, x, classify=False):
        ...
        print("         Input size to ViT:", x.size())
        x = self.prepare_tokens(x)
        print("         Size after token preparation:", x.size())
        ...
        print("         Output size of ViT:", x.size())
        return x


def vit_tiny(patch_size=(7, 16, 16), **kwargs):
...
def vit_small(patch_size=(7, 16, 16), **kwargs):
    model = VisionTransformer(
...
class CLSHead(nn.Module):
    ...
    def forward(self, x):
        print("CLS Head ...")
        print("         Input size to CLS Head:", x.size())
        x = self.mlp(x)
        print("         Output size of CLS Head:", x.size())
        return x
...
class RECHead_3D(nn.Module):
    ...
    def forward(self, x):
        ...
        print("         Input size to REC Head:", x.size())
        x = self.mlp(x)
        ...
        print("         Input size to the transposed convolution:", x_rec.unflatten(2, out_sz).size())
        x_rec = self.convTrans(x_rec.unflatten(2, out_sz))
        print("         Output size of REC Head:", x_rec.size())
